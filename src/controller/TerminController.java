         ...
         static boolean destroy(Date datum,int sifra_termina) throws Exception {
            ...
            try (Connection uspostaviKonekciju = SQLController.kreirajKonekciju()) {
                ...
                stmt1.setDate(1, (java.sql.Date) datum);
                 ...
                 }else if(JOptionPane.showConfirmDialog(null,"Da li ste sigurni da želite obrisat termin: "+datum,"Upozorenje!!!", JOptionPane.YES_NO_OPTION)==0){
            
                    PreparedStatement stmt = uspostaviKonekciju.prepareStatement(sql);
                    stmt.setInt(1,sifra_termina);
                    stmt.executeUpdate();
                    stmt.close();
                    System.out.println("DEBUG|TerminController|destroy|TERMIN "+datum+" OBRISAN!!!!!!!");
