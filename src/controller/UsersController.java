...
public class UsersController {
    ...
    this.sifra=sifra;
        ...
        try (Connection uspostaviKonekciju = SQLController.kreirajKonekciju()) {
            login = false;
            String sql =("SELECT COUNT(*) FROM users WHERE email=? AND password_2=?;");
        ...
        static  Users dohvatiKorisnika(String username, String password) throws Exception {
               ...
               try (Connection uspostaviKonekciju = SQLController.kreirajKonekciju()) {
                   ...
                   String sql = ("SELECT * FROM users WHERE email=? AND password_2=?;");
        ...
        static  int provjeraStudentaUEvidenciji(String brojIskaznice,int broj,Date datum) throws SQLException {
                  int pronadjeni = 0;
        ...
        public int provjeriEvidencijuZaStudenta(String broj_iskaznice,int broj,Date datum) throws Exception{
            int provjeri = provjeraStudentaUEvidenciji(broj_iskaznice,broj,datum);
            return provjeri;
        }
        ...
        //Dohvacanje kolegija iz baze po ID-u profesora
        public static Collection dohvatiEvidentiraneStudente(int sifra_kolegija,Date termin) throws Exception{

           ArrayList<Users> lista = new ArrayList<>();
            try{
                Connection uspostaviKonekciju = SQLController.kreirajKonekciju();
                
                String sql ="SELECT users.* FROM evidencija\n"
                           + "LEFT OUTER JOIN student_na_kolegiju ON evidencija.sifra_studenta_na_kolegiju=student_na_kolegiju.sifra_studenta_na_kolegiju\n"
                           + "LEFT OUTER JOIN users ON student_na_kolegiju.sifra_korisnika=users.sifra_korisnika\n"
                           + "WHERE student_na_kolegiju.sifra_kolegija=?\n"
                           + "AND evidencija.datum_evidentiranja=? AND evidencija.prisutnost=1 ORDER BY users.prezime;";
                ...
                while (rs.next()){
                    ...
                    String ime= rs.getString("ime");
                    String prezime= rs.getString("prezime");
                    lista.add(new Users(sifra_korisnika,broj_iskaznice,ime,prezime));
        ...
        public Collection listaEvidentiranihStudenata(int sifra_kolegija,Date termin) throws Exception{
            return dohvatiEvidentiraneStudente(sifra_kolegija,termin);
        }
        ...
        static boolean obrisiStudentaSTerminaPredavanja(int sifra_korisnika,Date termin,int sifra_kolegija) throws Exception {
            ...
            try (Connection uspostaviKonekciju = SQLController.kreirajKonekciju()) {
                ...
                stmt1.setInt(2,sifra_kolegija);
                 ...
                 if(JOptionPane.showConfirmDialog(null,"Da li ste sigurni da želite obrisat studenta s termina: "+termin,"Upozorenje!!!", JOptionPane.YES_NO_OPTION)==0){
